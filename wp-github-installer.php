...
class GitHub_Installer {
	...
	function __construct() {
		...
		add_action( 'upgrader_process_complete' , array( &$this , 'after_plugin_upgrade' ) );
		// do this only on 
	...
	function plugin_meta($plugin_meta, $plugin_file, $plugin_data, $status){
		if ( $this->is_github_plugin( $plugin_file, $plugin_data ) ) {
			array_unshift($plugin_meta,'<img src="https://github.com/favicon.ico" />');
			$repo = new GitHub_Repo($plugin_data['PluginURI']);
			var_dump($repo->get_latest_commit());exit();
		}
	...
	function after_plugin_upgrade( ) {
		// hmmm what to do..?
	}
	/*
	function get_download_url( $url ) {
		return $this->get_repository_url( $url ).'/archive/master.zip';
	}
	function get_plugin_slug( $url ) {
		return preg_replace('/(.*)\/([^\/]+)\/?$/','\2',$this->get_repository_url( $url ));
	}
	function get_repository_url( $url ) {
